<script lang="ts">
  ...
  async function secondMarketButton() {
    window.open("https://magiceden.io/marketplace/CfKrXFTW4UgLDeSSFsWftX6jZjbN1CMwuzh6TA4kcWbn", "_blank");
  }
...
<div class="flex flex-col">
  {#if !solana}
    <!-- <button
      class=" px-3 py-2 rounded-md  border-2 text-white font-bold"
      on:click={() => getPhantomWallet()}>Get Phantom Wallet</button
    > -->
    <button
      class=" px-3 py-2 rounded-md  border-2  text-white font-bold"
      on:click={secondMarketButton}>Buy On Secondary Market</button
    >
  ...
  {:else if $userState.userBalance < nftPrice()}
    <button
      class=" px-3 py-2 rounded-md  border-2 text-white font-bold disabled:bg-black-400"
      disabled={true}
