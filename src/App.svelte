<script lang="ts">
  ...
  const TITLE = "DESPERATE FACE";
  const DESCRTIPTION = "When the application was not accepted, one of them said, \"@formfunction definitely not my pool\", I said, \"@solana is the ocean\", and some people give up. Spontaneously I deploy this #SolanaNFTs collection, the DESPERATE FACE in SOLANA. maybe this can describe the faces of those who give up and hopefully, this motivates them.";
  const HEADER_TITLE = "DESPERATE FACE";
  const HEADER_LINK = "https://deface.endhonesa.com";
  // Your image or GIF needs to be in the /public folder for this to work
  const IMAGE_LINK = "/deface-sol.gif";
  /***********************************/
...
<main class="h-screen">
  ...
  {:else}
    ...
    {#if HEADER_TITLE}
      <a
        href={HEADER_LINK}
        class="text-white tracking-widest underline underline-offset-6 decoration-2 font-mono"
        >{HEADER_TITLE}</a
      >
      <p class="text-white">- human FACE when DESPERATE in SOLANA -</p>
    {/if}
    <!-- Card -->
    <div
      class=" max-w-lg mx-auto bg-grey rounded-lg my-12  border-2"
      transition:fade
    >
      <!-- Top Bar -->
      <Header />
      ...
      <div class="p-1 pb-4">
        ...
        <div
          class=" text-white text-lg sm:text-2xl font-mono font-bold py-5 tracking-wider"
        >
          {TITLE}
        </div>
        ...
        <div class="flex flex-col pt-3">
          {#if $userState.solanaExplorerLink}
            ...
            >
          {/if}
          <p class="text-white ">This collection part of <a title="" target="_blank" class="underline underline-offset-6">The KING's NFT Project</a></p>
        </div>
      </div>
    </div>
  {/if}
